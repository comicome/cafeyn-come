...
export default new Vuex.Store({
  ...
  getters: {
    filteredPublicationsCategory: state => {
      return state.selectedCategory === 'Accueil' ? state.publications.issues : filter(state.publications.issues, (issue: any) => issue.publication.category.name ===  state.selectedCategory)  
    },
    filteredPublicationsSubCategory: state => {
      return state.selectedCategory === 'Tout' ? state.publications.issues : filter(state.publications.issues, (issue: any) => issue.publication.category.subCategories[0].name ===  state.selectedSubCategory)
    }
  },
  mutations: {
